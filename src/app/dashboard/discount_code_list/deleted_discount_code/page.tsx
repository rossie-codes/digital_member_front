...
const DeletedDiscountCodePage: React.FC = () => {
    ...
    const fetchDeletedDiscountCodes = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/discount_code/get_deleted_discount_code_list`,
          {
            method: "GET",
            headers: {
        ...
        if (response.status === 404) {
          ...
          throw new Error(
            `Failed to fetch deleted redemption items: ${response.status}`
          );
        } else {
        ...
        message.error(
          `Error fetching deleted redemption items: ${error.message}`
        );
      } finally {
        setLoading(false);
      }
      ...
      render: (_, record) => (
        <Space>
          <Button
            type="link"
            icon={<UndoOutlined />}
            onClick={() => handleRestoreItem(record.discount_code_id)}
          >
            Restore
          </Button>
  ...
  const handleRestoreItem = async (id: number) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/discount_code/restore_discount_code/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
